...
/opt/mssql-tools/bin/sqlcmd -H localhost -U $BACKUP_USER -P $BACKUP_PASS -Q "set nocount on; select name from sys.databases where not name in ('master', 'tempdb', 'model', 'msdb')" -h -1 | while read dbname; do
